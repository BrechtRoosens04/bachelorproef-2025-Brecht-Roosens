...
Als gelabelde fouten beperkt of helemaal ontbreken, kunnen unsupervised learning-methoden een geschikt alternatief bieden voor supervised learning. In tegenstelling tot supervised modellen leren deze algoritmen alleen de eigenschappen van normaal machinegedrag, zonder gebruik te maken vanop voorhand gelabelde defecten. De onderzoeken van \textcite{Carvalho2019} en \textcite{Giannoulidis2024} stellen dat afwijkingen van dit gebruikelijke gedrag daarna worden gezien als mogelijke anomalieën die kunnen duiden op een eerste storing of defect.
...
Recent onderzoek toont daarnaast aan dat diverse unsupervised en deep learning-methoden met succes worden ingezet voor het opsporen van anomalieën in tijdsreeksen. Voorbeelden hiervan zijn Isolation Forest-model, Autoencoders, LSTM Autoencoders en One-Class Support Vector Machines, die elk op een andere wijze het gebruikelijke gedrag van machines modelleren en afwijkingen opsporen \autocite{Carvalho2019, Elahi2023, Giannoulidis2024}. Deze methoden zijn de fundamenten van de machine learning-methodiek die in dit onderzoek wordt beoordeeld.
...
Het Isolation Forest-model is een unsupervised machine learning-algoritme dat speciaal is ontworpen voor het opsporen van afwijkingen. In tegenstelling tot supervised modellen heeft het Isolation Forest-model tijdens de training geen gelabelde fouten nodig. Het algoritme veronderstelt dat afwijkende observaties eenvoudiger kunnen worden geïsoleerd dan gebruikelijke observaties. Volgens het onderzoek van \textcite{Carvalho2019} moet er voor dit doel willekeurige beslissingsbomen gevormd worden, waarbij afwijkingen meestal met minder splitsingen worden gerealiseerd dan gebruikelijke datapunten.

Binnen voorspellend onderhoud wordt het Isolation Forest-model steeds meer gebruikt om afwijkingen in sensorgegevens te identificeren wanneer historische foutlabels beperkt of niet toegankelijk zijn. Volgens het onderzoek van \textcite{Carvalho2019} is het Isolation Forest-model één van de meest populaire methoden voor het opsporen van onzichtbare afwijkingen, door de eenvoudige uitvoering, de korte rekentijd en de capaciteit om grote hoeveelheden sensordata effectief te bestuderen.

Unsupervised methoden, zoals het Isolation Forest-model, zijn bijzonder geschikt voor industriële toepassingen waarin defecten slechts af en toe voorkomen en gelabelde foutgegevens beperkt zijn \autocite{Elahi2023}. Volgens de studie van \textcite{Giannoulidis2024} is het ook mogelijk om anomalieën te detecteren op basis van normaal machinegedrag, wat een waardevolle oplossing biedt voor voorspellend onderhoud wanneer alleen sensordata van de gebruikelijke machinecondities beschikbaar zijn.

Een voordeel van het Isolation Forest-model is dat het zonder voorafgaande kennis van storingen of defecten in staat is om afwijkende patronen te herkennen. Het model is bovendien relatief snel te trainen en kan goed worden geschaald naar grotere datasets. Een nadeel is dat het algoritme afwijkingen opmerkt op basis van statische isolatie, wat betekent dat niet elke waargenomen afwijking noodzakelijk gelijkstaat aan een daadwerkelijk defect. Volgens de onderzoeken van \textcite{Carvalho2019} en \textcite{Elahi2023} zijn de prestaties daarnaast afhankelijk van de gekozen modelparameters en de kwaliteit van de beschikbare sensordata.

In dit onderzoek wordt het Isolation Forest-model beschreven als het eerste unsupervised referentiemodel voor het opsporen van anomalieën. Het model wordt gebruikt voor zowel de openbare MetroPT-3-dataset als de private tractordataset om te onderzoeken in hoeverre afwijkingen kunnen worden vastgelegd zonder gebruik te maken van gelabelde foutdata. Dit maakt het mogelijk om de toepasbaarheid van deze techniek in kleine bouwbedrijven te evalueren.
...
\section{\IfLanguageName{dutch}{One-Class SVM-model}{One-Class SVM-model}}%
\label{sec:{One-Class SVM-model}
    
De One-Class Support Vector Machine-model of One-Class SVM-model is een unsupervised machine learning-algoritme dat is ontwikkeld voor het opsporen van anomalieën. In tegenstelling tot een tradtionele Support Vector Machine-model, die klassen van elkaar onderscheidt, leert een One-Class SVM-model enkel de eigenschappen van gebruikelijke data. Volgens het onderzoek van \textcite{Carvalho2019} stelt het model een limiet vast voor de gebruikelijke observaties, waarna nieuwe waarnemingen die daarbuiten vallen als afwijkingen worden gezien.

Bij voorspellend onderhoud is de One-Class SVM-model bijzonder waardevol wanneer er alleen normale conditiedata beschikbaar zijn en defecten nauwelijks voorkomen. De One-Class SVM-model is één van de meest populaire unsupervised methoden voor het detecteren van fouten in industriële toepassingen, omdat het model geen gelabelde foutdata vereist om afwijkingen te herkennen \autocite{Carvalho2019}. Dit biedt een nuttig alternatief in situaties waarin historische defectgegevens beperkt of helemaal ontbreken.

De werking van een One-Class SVM-model hangt echter sterk af van de keuze van de kernel en de hyperparameters, waaronder de parameter $\nu$ en de kernelparameter $\gamma$. Bovendien is het model minder geschikt voor zeer grote datasets en ingewikkelde multivariate tijdsreeksen dan deep learning-modellen \autocite{Elahi2023}. Volgens de studie van \textcite{Giannoulidis2024} zijn neurale netwerken zoals Autoencoders meestal effectiever wanneer er ingewikkelde patronen in sensordata moeten worden ontdekt, terwijl One-Class SVM-model vooral geschikt is voor kleinere datasets of minder ingewikkelde anomaliedetectie-taken.

Een significant voordeel van de One-Class SVM-model is dat het model relatief gemakkelijk te implementeren is en geen uitgebreide trainingsgegevens met foutlabels vereist. Dit maakt het mogelijk om het snel te gebruiken als basismodel voor het opsporen van anomalieën en biedt het een belangrijke referentie bij het vergelijken van diverse unsupervised leermethoden.

In dit onderzoek wordt de One-Class SVM-model gebruikt als een klassiek unsupervised model voor het detecteren van anomalieën. Het model wordt eerst beoordeeld op basis van de openbare MetroPT-3-dataset en daarna geïmplementeerd op de private tractordataset. De uitkomsten worden vergeleken met die van Random Forest-model, Isolation Forest-model, Autoencoder en LSTM Autoencoder om te bepalen welke methode het beste werkt voor het opsporen van afwijkingen in sensorgegevens en het ondersteunen van het onderhoudsbeleid voor bouwmachines in kleine bouwbedrijven.
...
\section{\IfLanguageName{dutch}{De beschikbaarheid en de kwaliteit van sensorgegevens}{The availability and quality of sensor data}}%
\label{sec:{De beschikbaarheid en de kwaliteit van sensorgegevens}
    
De effectiviteit van machine learning-modellen voor het voorspellen van onderhoud hangt sterk af van de beschikbaarheid en de kwaliteit van senordata. Voor een betrouwbare detectie van afwijkingen en defecten zijn er genoeg historische gegevens vereist die zowel gebruikelijke als ongebruikelijke machinecondities weergeven. Volgens de onderzoeken van \textcite{Carvalho2019} en \textcite{Elahi2023} blijkt dat in de praktijk het verzamelen van dergelijke datasets echter een aanzienlijke uitdaging te zijn.

Veel industriële organisaties hebben slechts een beperkte hoeveelheid gelabelde foutdata, omdat defecten relatief weinig voorkomen en onderhoudsinformatie niet altijd op een systematische manier wordt vastgelegd \autocite{Elahi2023}. Bovendien omvatten sensordatasets vaak ruis, onjuiste waarden en meetfouten, wat de prestaties van machine learning-modellen kan schaden. Volgens het onderzoek van \textcite{Wang2025}  kunnen ruis en signaalvervorming ook een grote impact hebben op de betrouwbaarheid van het opsporen van anomalieën en het stellen van een foutdiagnose.

Binnen de bouwsector wordt deze kwestie nog verergerd door de zware werkomstandigheden waarin machines opereren. Bouwmachines worden onderhevig aan variabele belastingen, stof, trillingen en verschillende weersomstandigheden, wat kan leiden tot aanzienlijke variaties in de sensordata. De studie van \textcite{Mikulic2024} stelt ook dat kleine bouwbedrijven meestal minder geavanceerde monitoring- en dataverzamelingssystemen dan grote industriële ondernemingen, wat leidt tot het ontbreken van kwalitatieve historische datasets.
    
In de recente literatuur worden diverse oplossingen aangedragen om het gebrek aan representatieve data te verhelpen. Het toepassen van synthetische gegevens en digitale twins om machine learning-modellen te ontwikkelen en te verifiëren wordt gedaan wanneer historische sensorgegevens beperkt beschikbaar zijn \autocite{Meddaoui2024, Hu2024}. Bovendien laat het onderzoek van \textcite{Nieminen2026} zien dat synthetische datasets steeds vaker worden gebruikt voor voorspellend onderhoudsprocessen in Industry4.0-toepassingen.

Binnen dit onderzoek is de beperkte beschikbaarheid van kwalitatieve sensordata eveneens een aanzienlijke uitdaging. Hoewel het oorspronkelijke doel was om een voorspellend onderhoudsmodel te creëren voor een kleine bouwonderneming, werd tijdens het verzamelen van data vastgesteld dat er onvoldoende gelabelde defect- en onderhoudsdata beschikbaar waren. Er is daarom gekozen om onderzoek te doen waarbij diverse anomaliedetectie-modellen eerst worden beoordeeld op een openbare dataset en daarna worden toegepast op een privé-tractordataset om te kijken welk model het beste zou passen in de methodiek. Zo kan de bruikbaarheid van de ontwikkelde anomaliedetectie-model worden bestudeerd in een realistische praktijksituatie.

\section{\IfLanguageName{dutch}{Synthese en de keuze van de bestudeerde modellen}{Synthesis and selection of the models studied}}%
\label{sec:{Synthese en de keuze van de bestudeerde modellen}
    
Volgens de literatuur hangt de keuze voor een machine learning-model voor voorspellend onderhoud sterk af van de beschikbaarheid en kwaliteit van de sensordata. Als er genoeg gelabelde fouten beschikbaar zijn, leveren supervised learning-modellen vaak uitstekende resultaten op. De studies van \textcite{Carvalho2019} en \textcite{Elahi2023} stellen dat veel bedrijven in de praktijk echter slechts een beperkte hoeveelheid historische defectgegevens hebben, wat de relevantie van dergelijke modellen vermindert.

De literatuur laat zien dat het opsporen van onregelmatigheden in dergelijke situaties een geschikt alternatief biedt. Modellen zoals Isolation Forest-model, Autoencoders, LSTM Autoencoders en One-Class SVM leren het gebruikelijke gedrag van een machine en zijn in staat om afwijkingen te herkennen zonder duidelijke foutlabels \autocite{Giannoulidis2024, Davari2021}. Deze technieken zijn daardoor zeer aantrekkelijk voor kleine bedrijven, waar storingen realtief zelden voorkomen en historische onderhoudsinformatie vaak niet volledig is.

In dit onderzoek zijn vijf modellen gekozen op basis van de literatuur. Random Forest-model is ingedeeld als representatief supervised referentiemodel, omdat dit algoritme vaak wordt gebruikt in het voorspellend ondrhoud en goede prestaties levert wanneer er genoeg gelabelde data beschikbaar zijn \autocite{Carvalho2019}. Bovendien zijn er vier modellen voor het detecteren van anomalieën bestudeerd: Isolation Forest-model, Autoencoder, LSTM Autoencoder en One-Class SVM. Volgens de odnerzoeken van \textcite{Elahi2023} en \textcite{Giannoulidis2024} zijn deze modellen niet afhankelijk van uitgebreide foutlabels, waardoor ze beter aansluiten bij de problemen die in de literatuur worden genoemd voor industriële toepassingen met beperkte datasets.

De gekozen modellen worden eerst beoordeeld op basis van de publieke MetroPT-3-dataset, die speciaal is ontworpen voor onderzoek naar voorspellend onderhoud en het opsporen van anomalieën \autocite{Veloso2022}. Daarna wordt dezelfde methodiek toegepast op een privédataset van een tractor die dient voor grondverzet en transport. Dit maakt het mogelijk om te onderzoeken in hoeverre de ontwikkelde methode toepasbaar is op een realistische dataset uit de bouwsector.

De literatuur toont aan dat er geen enkel universeel model is dat in elke situatie de beste resultaten oplevert. De selectie van een passend model is afhankelijk van de beschikbare sensorgegevens, de aanwezigheid van foutenlabels en de eigenschappen van de toepassing. Dit onderzoek concentreert zich op het creëren en beoordelen van een praktische machine learning-methodiek die kleine bouwbedrijven kan helpen bij het opsporen van mogelijke afwijkingen in de sensorgegevens maar ook met het ondersteunen van hun onderhoudsbeleid. De analyse van vijf diverse modellen dient aan te tonen welk model het meest geschikt is voor dit doel en in welke mate deze aanpak kan worden toegepast op zowel een openbare als een private datasets.
